<script lang="ts">
	import type { getEventBySlug } from '$lib/data/sanity/events';
...
<aside class="w-full md:max-w-[350px] space-y-4">
	{#if event.isPrivate}
		<div class="bg-red-500 text-white p-4 rounded-xl border-2 shadow-xl">
			<p class="font-medium text-center">Dette arrangementet er privat</p>
		</div>
	{/if}

	<div class="-full h-fit p-6 border-2 bg-background rounded-xl shadow-xl">
		<ul class="space-y-8">
			<li>
			...
			{#if event.registrationLink}
				<li>
					<a
						class="bg-primary rounded py-2 px-4 font-medium border-2 h-10 flex justify-center items-center hover:bg-primary-light transition-colors"
